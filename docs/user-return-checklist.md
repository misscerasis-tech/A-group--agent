...
12. 在还没贴经营表前，先发 `我现在做什么`。机器人应该给 Agent 接手步骤，并带一张可复制的最小经营表。

13. 继续发 `给我本周运营计划`、`生成周报包`、`哪些需要人工确认`、`风险提醒怎么发`。机器人应该返回排期、周报产物、审核队列和提醒规则。

14. 在同一个飞书会话里粘贴一段经营表，再问 `给我待办清单`。worker 应该使用刚粘贴的数据。然后继续粘贴广告表、库存/成本表、用户声音表或竞品表，机器人应该提示“已合并到当前会话数据”。再发 `清空这份数据`，它应该清除当前 chat 缓存；之后重新问待办时，会回到 `.env` 数据或样例店铺。
...
如果不知道表头怎么写，先点每个表格右上角的“复制模板”，把模板贴到 Excel、飞书表格或聊天里，改成真实商品和数字后再粘回工作台。
