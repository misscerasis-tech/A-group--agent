...
当前导入器已经支持 CSV/TSV/Markdown 表格，不要求字段名完全一致。从 Excel、飞书表格或 Google Sheets 直接复制出来的制表符表格也能识别。网页工作台也支持上传 `.xlsx/.xls`，会优先读取第一张像表格的数据工作表；如果前面是空 sheet 或只有说明文字的 sheet，会继续往后找，再转成 CSV 文本进入同一套字段识别流程。常见别名会自动识别：

`week` 可以直接写 `previous/current`、`上周/本周`、`上期/本期`，也可以写真实日期、中文日期、日期区间或周，例如 `2026-07-08`、`2026年7月13日`、`2026-07-13 - 2026-07-19`、`2026-W29`。如果没有单独的 `startDate/endDate`，Agent 会从周期里的日期推导本周起止日期，用来判断竞品观察是否过期。如果没有 `week` 列，但有 `date/start_date/开始日期/统计日期`，Agent 会把日期当成周期来源。如果导出的表里超过两个周期，Agent 会自动选择最近两期做对比，并在导入报告里说明它选了哪两期。

如果从聊天、文档或平台后台复制出来的表格前后带人话说明，例如“导出时间：2026-07-23”或表尾“备注：以上数据仅供参考”，Agent 会跳过这些明显不是数据行的说明，继续按真实表头和数据行分析。

数字不用手动清洗成纯数字。常见平台写法例如 `约90单`、`1.2万元+`、`2.6千元左右`、`3百件`、`80元`、`US$1,200.50`、`USD 80.25`、`RMB 90`、`￥980.00`、`€1.234,56`、`1 280,40 €` 都能识别。毛利等允许为负的字段也支持会计写法，例如 `(30.5)` 或 `(€30,50)` 会按 `-30.5` 处理。退款金额如果被平台导出成 `-39.9` 或 `(39.9)`，Agent 会按绝对值当作退款金额，并在导入报告里提醒这个口径；订单数、销售额、销量、库存等经营指标仍不能为负数。平台如果只导出 `客单价`、`转化率`、`ROAS/投产比`、`毛利率`、`退款率` 或 `退款金额占比`，Agent 会在不覆盖原始字段的前提下反推销售额、访客数、广告成交额、毛利、退款/退货单数或退款金额，用来继续做复盘。
...
如果平台导出的经营表、广告表、库存/成本快照、用户声音表或竞品表里带 `总计`、`合计`、`汇总`、`小计`、`Total` 或 `Grand Total` 行，Agent 会自动跳过这些汇总行，避免把汇总额当成一个商品、竞品或反馈主题重复参与分析。

如果导出表里出现重复表头，例如两列都叫 `revenue` 或 `销售额`，Agent 会保留第一列原名，把后面的同名列临时改成 `revenue__2` 这类后缀，避免数据被悄悄覆盖。字段识别优先使用第一列已知表头；如果平台把真正要用的值放在后面的重复列，建议用户先改一下表头再导入。
...
| productName | product_name、product、listing_title、商品名称、商品标题、宝贝标题、产品标题、品名 |
| sku | sku、seller_sku、asin、product_id、item_id、商品ID、宝贝ID、商家编码、商品编码、货号、款号 |
| visitors | visitors、sessions、uv、访客数、商品访客数、流量 |
| conversionRate | conversion_rate、cvr、转化率、支付转化率、成交转化率 |
| orders | orders、order_count、订单数、支付订单数、支付买家数、成交笔数、付款人数 |
| revenue | revenue、sales、gmv、net_sales、total_sales、gross_sales、销售额、净销售额、总销售额、成交额、支付金额、商品支付金额、买家实付、实付金额 |
...
| unitsSold | units_sold、net_quantity、quantity、qty、items_sold、total_items、销量、支付件数、支付商品件数、净销量 |
| adSpend | ad_spend、cost、广告花费、广告消耗、推广消耗、广告支出、消耗 |
| adRevenue | ad_revenue、attributed_sales、广告成交额、广告销售额、广告GMV、广告订单金额、直接成交金额 |
...
如果 `priceNote` 标注“无 featured offer”“无库存”“历史价”“仅用于价格带”或“不作为主动调价依据”，Agent 只会把这条竞品当作观察线索，不会把它算进主动降价判断。`promotion` 字段也只建议填写真实优惠，例如折扣、券、满减、赠品或 sale price；商品定位和卖点请放到 `keySellingPoints`。

如果 `observedAt` 距离本次复盘周期超过 30 天，即使价格字段存在，Agent 也会把这条竞品价格和促销当作过期观察线索。调价或跟促销前，建议重新打开竞品链接，更新观察日期、当前价格和价格备注。
