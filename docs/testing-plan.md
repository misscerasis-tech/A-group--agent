...
- 上传国内平台导出的 GB18030/GBK 中文 CSV 时，表头不应该变成乱码；如果仍无法读取，页面应该提示复制表格内容粘贴。
- 粘贴很大的平台导出表时，如果浏览器缓存草稿失败，页面应该提示“本次分析不受影响”，并继续允许生成复盘。
- 生成基于导入数据的复盘和飞书回写文本。
- 检查“下周行动”是否包含负责人、优先级、截止时间和验收标准，并可复制为待办表格。
- 检查“风险商品表”是否包含排查状态、建议负责人、SKU、问题、人话原因和建议动作，并可复制到飞书表格。
...
- 如果粘贴表格前后带说明行或表尾备注，例如“导出时间”或“以上数据仅供参考”，Agent 会跳过这些非数据行。
- 上传 `.xlsx/.xls` 时，网页工作台会优先读取第一张像表格的数据工作表并转成表格文本；第一页空白或只有说明文字的 sheet 不会阻断导入。
- 没有 `week` 列但有 `date/start_date/开始日期` 时，Agent 会用日期判断最近两期；如果周期列里写的是 `2026年7月13日` 或 `2026-07-13 - 2026-07-19`，Agent 也会推导本周起止日期。
...
- 如果经营表、广告表、库存/成本快照、用户声音表或竞品表里带 `总计/合计/汇总/Total/Grand Total` 行，Agent 会跳过这些汇总行，不会把它们当成商品、竞品或反馈主题重复计算。
...
- 竞品 `observed_at` 如果距离本次复盘周期超过 30 天，这条竞品的价格和促销也只能作为观察线索，不能触发主动降价或跟促销建议。
...
飞书事件处理会在回复发送成功后才记录去重；如果发送接口临时失败，飞书重试同一事件时仍会再次尝试回复。
...
Shopify Orders 和 Amazon 订单 TSV 的原始表头也可以直接粘贴到飞书；机器人应识别 `Paid at / Lineitem price`、`amazon-order-id / purchase-date / item-price` 这类字段，并按订单明细聚合后回复。

如果粘贴表格前后带了一两行说明，例如“这是我从飞书表格复制的经营数据”“导出时间：2026-07-23”或“备注：以上数据仅供参考”，机器人也应该跳过说明行和表尾备注，自己找到真实表头和数据行。
如果说明行后面的数据有异常，例如订单数为负数，机器人应该提示用户修正原始粘贴内容里的真实行号，而不是按跳过说明后的表格行重新编号。

如果直接给机器人发文件、图片或富文本，当前长连接不会下载附件。机器人应该明确告诉用户：复制表头和几行数据直接粘贴，或到 `/agent` 工作台上传 CSV/Excel 后再把结果发回飞书。

第一版会用样例店铺回复，验证闭环后再把输入换成真实表格。
如果在同一个飞书会话里先粘贴经营表，再发 `给我待办清单`、`我还缺什么数据` 或 `帮我看本周经营情况`，本地 worker 应该继续使用刚粘贴的数据。默认情况下，这个 chat 上下文会保存在本机 `.agent-state/feishu-chat-contexts.json`，worker 重启后仍可恢复；该目录已被 Git 忽略。若要测试“无缓存”状态，先设置 `FEISHU_CHAT_CONTEXT_PERSISTENCE=off` 或删除 `.agent-state/feishu-chat-contexts.json`。
如果用户发 `清空这份数据`、`忘记当前数据` 或 `重新开始`，worker 应该删除当前 chat 的缓存。之后再问 `给我待办清单` 时，应回到 `.env` 配置数据或样例店铺，而不是继续用刚才粘贴的表。

迁移能力测试：

```bash
npx pnpm@10.13.1 run feishu:migration-plan
npx pnpm@10.13.1 vitest run src/lib/integrations/feishu/migration.test.ts
```

通过标准：

- `feishu:migration-plan` 在缺少新连接信息时打印非敏感配置模板，在补齐新旧 App ID 后生成切换前检查和回滚步骤。
- 不在代码里写死 App Secret、chat ID 或文档 ID。
- 新旧连接必须属于同一个 Workspace。
- 迁移前必须有测试步骤。
- 回滚路径明确。
