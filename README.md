...
- 把同一份分析同步成运营计划、周报包、审核队列、风险提醒和经营复盘页面，避免真实试用时只有一个演示页可看。
- 结果回写飞书群、文档或待办。
...
首页已经提供“真实数据导入工作台”，可以直接粘贴或上传 CSV/TSV/Markdown/Excel 表格，也可以从 Excel、飞书表格或 Google Sheets 复制表格粘贴进来。上传文本 CSV/TSV 时，如果 UTF-8 解码出现乱码，会尝试 GB18030/GBK 兜底，兼容更多国内平台中文导出。上传 `.xlsx/.xls` 时会优先读取第一张像表格的数据工作表；如果前面是平台说明页或空 sheet，会继续往后找。经营数据既可以是按上周/本周汇总好的 SKU 表，也可以是覆盖最近两周的订单明细；Shopify Analytics 这类汇总表里的 `net_sales / total_sales / net_quantity`，以及只给 `AOV / 客单价` 的汇总表，也能先进入复盘。Shopify Orders 导出的 `Name / Paid at / Lineitem name / Lineitem sku / Lineitem quantity / Lineitem price / Discount Amount`、Amazon Seller Central 订单 TSV 导出的 `amazon-order-id / purchase-date / sku / quantity-purchased / item-price` 这类明细表也能直接聚合。订单明细里的 `discount_amount` 会先从收入中扣除，平台佣金、支付手续费和履约费会进入利润口径。工作台会显示 Agent 接手步骤、经营判断地图、下一句要追问什么、下一份要补的数据、字段识别结果和飞书回写预览；当用户还没有经营表或缺关键字段时，接手步骤会直接给出可复制的最小经营表。补数清单可以复制成 TSV，方便贴到飞书表格或多维表格。API 会返回 `tableTemplates` 和 `operationalWorkspace`，外部入口可以直接把模板、排期、审核队列、提醒、周报和待办发给用户。金额展示按原表金额单位理解，不默认加 `$` 或 `¥`，也不自动换算币种。飞书 worker 也可以通过 `.env` 的 `ECOMMERCE_WEEKLY_METRICS_CSV`、`ECOMMERCE_COMPETITORS_CSV`、`ECOMMERCE_ADS_CSV`、`ECOMMERCE_INVENTORY_CSV` 和 `ECOMMERCE_CUSTOMER_VOICES_CSV` 指向本地经营表、竞品表、广告表、库存/成本快照表和用户声音表文件。
...
回到电脑后先看当前版本、样例数据、真实数据配置、飞书配置缺口和本地页面状态：
...
飞书里也可以直接问“给我本周运营计划”“生成周报包”“哪些需要人工确认”“风险提醒怎么发”，机器人会按当前导入数据返回排期、周报产物、审核队列和提醒规则。
