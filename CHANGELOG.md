...
- 飞书机器人新增“运营计划、周报包、审核队列、风险提醒”四类自然语言入口，用户可以直接问 `给我本周运营计划`、`生成周报包`、`哪些需要人工确认`、`风险提醒怎么发`。
- 飞书回复测试和 Agent smoke 覆盖这些新入口，确认它们使用当前导入数据或样例降级生成同一套运营工作面。
...
- 经营概览页不再给销售额硬加 `$`，并在页面烟测中检查“金额单位按原表理解”的口径，避免真实人民币、美元或欧元数据被误标。

## [v0.1.147-operational-workspace-pages] - 2026-07-23

### Added

- 新增运营工作面派生模块，把同一份 Agent 分析结果转成运营排期、审核队列、风险提醒、周报产物和复盘指标解释。
- 运营计划、周报包、审核中心、风险提醒和经营复盘页面不再是“后续开放”占位，改为无需数据库也能展示可执行工作的页面。
- 页面烟测新增这些工作面的关键文案检查，防止回退成空页面或漏出 Prisma / `DATABASE_URL` 原始错误。

## [v0.1.146-stale-competitor-import-coverage] - 2026-07-23

### Added

- 分析单测增加从 CSV 导入中文日期周期后再判断竞品观察过期的端到端覆盖，确认日期只在周期列里时也不会把过期低价/促销当成主动降价依据。
