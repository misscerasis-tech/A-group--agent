...
const inventoryTable = ["商品名称,商家编码,当前库存,库存日期", "黑杯,CUP-BLACK,18,2026-07-19"].join("\n");
...
async function main() {
  ...
  assert(report.inventoryRows === 1, "接口应该识别库存快照表行数。");
  ...
  console.info(`[smoke:api] /api/agent/analyze 平台表头、订单明细、库存快照、缺参和缺字段检查均通过：${baseUrl}`);
}
